...
function invertCase(str) {
  ...
  return arr
    .map((it) => {
      return /[A-Z]/.test(it) ? it.toLowerCase() : it.toUpperCase();
    })
    .join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (code1.includes(str[i])) {
      const pos = code1.indexOf(str[i]);
      codeSent += code2[pos];
    } else if (code2.includes(str[i])) {
      const pos = code2.indexOf(str[i]);
      codeSent += code1[pos];
    } else {
      codeSent += str[i];
    }
